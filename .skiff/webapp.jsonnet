...
function(image, cause, sha, env, branch, repo, buildId)
    local id = std.strReplace(env, '_', '-');
    // A list of hostnames served by your application. By default your application's
    // `hello-world` environment will receive requests made to `$appName.apps.allenai.org` and
    // non-production environments will receive requests made to `$env.$appName.apps.allenai.org`.
    //
    ...
    local hosts = util.getHosts(env, config, '.apps.allenai.org');
    ...
    local numReplicas = config.replicas.prod;
    ...
    local fullyQualifiedName = config.appName + '-' + id;
